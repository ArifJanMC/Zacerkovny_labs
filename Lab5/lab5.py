...
for i in range(len(ciphers)):
    ...
    for j in range(len(ciphers)):
        if i!=j:
            B = bytes( [ a ^ b for a,b in zip(ciphers[i],ciphers[j])] )
            P = np.array([ int(b>=ord('A') and b<=ord('Z')) for b in B[:minCipherL] ])
            pairXOR[i].append( B )
            positions[i].append(P)
...
Source=[]
for i in range(len(ciphers)):
    Source.append( bytes( [ a ^ b for a,b in zip(ciphers[i],SecKey) ] ) )

for i in range(len(ciphers)):
    print(Source[i].decode(errors='replace'))

print("\nDecrypted target message:")
print(bytes([ a ^ b for a, b in zip(targ, SecKey) ]).decode(errors='replace'))
